...
exports.CruiseModule = void 0;
const types_1 = require("../../types");
// 默认巡航配置
const DEFAULT_CRUISE_CONFIG = {
    maxPositions: 5,
    minHealthScore: 3.0,
...
class CruiseModule {
     ...
     * 构造函数
     */
    constructor(logger, agentStateMachine, transactionExecutor, fundsManager, riskController, signalService, positionOptimizer, taskManager, metrics) {
        this.registeredAgents = new Map();
        this.agentCruiseConfigs = new Map();
    ...
    async performHealthCheck(agentId) {
        try {
            ...
            if (state !== types_1.AgentState.RUNNING) {
            ...
            // 识别不健康的仓位
            const unhealthyPositions = await this.positionOptimizer.identifyUnhealthyPositions(funds.positions, riskAssessmentResult);
            ...
            for (let i = 0; i < Math.min(availableSlots, sortedPools.length); i++) {
                ...
                try {
                    const result = await this.transactionExecutor.executeTransaction({
                        type: types_1.TransactionType.ADD_LIQUIDITY,
                        agentId,
                        data: {
     ...
     * 优化代理仓位
     */
    async optimizePositions(agentId) {
        try {
            if (!this.isRunning) {
            ...
            const riskAssessment = await this.riskController.assessRisk(agentId);
            // 获取优化建议
            const optimizationPlan = await this.positionOptimizer.calculateOptimalPositions(agentId, {
                ...funds,
            ...
            for (const action of optimizationPlan.actions) {
                try {
                    const transactionType = action.type === 'add' ? types_1.TransactionType.ADD_LIQUIDITY :
                        action.type === 'remove' ? types_1.TransactionType.REMOVE_LIQUIDITY :
                            types_1.TransactionType.ADD_LIQUIDITY; // 默认为添加流动性
                    const result = await this.transactionExecutor.executeTransaction({
     ...
     * 检查市场重大变化
     */
    async checkForSignificantChanges(agentId) {
        try {
            if (!this.isRunning) {
     ...
     * 记录健康检查指标
     */
    recordHealthCheckMetrics(agentId, data) {
        this.metrics.recordHealthCheck(agentId, true, // success
        0, // duration
     ...
     * 记录仓位填充指标
     */
    recordPositionFillMetrics(agentId, data) {
        this.metrics.recordOptimization(agentId, data.succeeded > 0, // success
        0, // duration
     ...
     * 记录优化指标
     */
    recordOptimizationMetrics(agentId, data) {
        this.metrics.recordOptimization(agentId, data.successfulActions === data.plannedActions, // success
        0, // duration
     ...
     * 设置代理的定时任务
     */
    async setupAgentTasks(agentId, config, cruiseConfig) {
        const tag = `agent:${agentId}`;
        // 设置健康检查任务
        this.taskManager.scheduleRecurringTask(`${tag}:health_check`, async () => {
            await this.performHealthCheck(agentId);
            return;
        }, cruiseConfig.healthCheckIntervalMinutes * 60 * 1000, 0, // startDelayMs
        [tag]);
        // 设置优化任务
        this.taskManager.scheduleRecurringTask(`${tag}:optimization`, async () => {
            await this.optimizePositions(agentId);
            return;
        }, cruiseConfig.optimizationIntervalHours * 60 * 60 * 1000, 0, // startDelayMs
        [tag]);
        // 设置市场变化检查任务
        this.taskManager.scheduleRecurringTask(`${tag}:market_check`, async () => {
            await this.checkForSignificantChanges(agentId);
            return;
        }, cruiseConfig.marketCheckIntervalMinutes * 60 * 1000, 0, // startDelayMs
        [tag]);
        this.logger.info(`Scheduled all tasks for agent ${agentId}`);
    }
}
exports.CruiseModule = CruiseModule;
//# sourceMappingURL=CruiseModule.js.map