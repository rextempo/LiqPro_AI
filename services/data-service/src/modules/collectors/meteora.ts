...
export class MeteoraPoolCollector {
  ...
  async getPoolInfo(poolAddress: string, forceRefresh: boolean = false): Promise<DLMMPoolInfo> {
    ...
    if (forceRefresh || !this.poolCache.has(poolAddress) || (now - lastUpdate > this.updateInterval)) {
      try {
   ...
   */
  async monitorLargeRemovals(poolAddress: string, thresholdPercentage: number = 10): Promise<any | null> {
    try {
