...
// 测试发布带有大价格变化的池数据到RabbitMQ（用于触发信号生成）
app.post('/api/test/publish-pool-data-with-signal', async (req, res) => {
  try {
    // 使用请求体中的数据，如果没有则使用默认值
    const testPoolData = req.body || {
      address: 'test-pool-address-signal',
      poolId: 'test-pool-id-signal',
      tokenX: 'SOL',
      tokenY: 'USDC',
      tokenA: 'SOL',  // 添加tokenA和tokenB字段，与signal-service中的字段匹配
      tokenB: 'USDC',
      price: 100.0,
      liquidity: 1000000,
      volume24h: 500000,
      priceChange24h: 0.1,  // 大于0.05的价格变化，以触发信号生成
      timestamp: new Date().toISOString()
    };
    
    // 确保有tokenA和tokenB字段
    if (!testPoolData.tokenA && testPoolData.tokenX) {
      testPoolData.tokenA = testPoolData.tokenX;
    }
    if (!testPoolData.tokenB && testPoolData.tokenY) {
      testPoolData.tokenB = testPoolData.tokenY;
    }
    
    logger.info(`发布带有大价格变化的测试池数据: ${JSON.stringify(testPoolData)}`);
