...
import { AgentCreateParams, AgentStatus } from '../clients/agent-client';
...
router.get('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const agentClient = ServiceManager.getInstance().getAgentClient();
...
router.post('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const params = req.body.params as AgentCreateParams;
...
router.put('/:id/status', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const agent = await agentClient.updateAgentStatus(id, status as AgentStatus);
 ...
 * 紧急退出
 */
router.post('/:id/emergency-exit', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const agentClient = ServiceManager.getInstance().getAgentClient();
