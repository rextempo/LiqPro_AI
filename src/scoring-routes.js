...
const logger = new logger_1.Logger('ScoringRoutes');
/**
 * 对单个信号进行评分
 */
router.post('/signal', async (req, res, next) => {
    try {
        ...
        const scoringClient = service_manager_1.ServiceManager.getInstance().getScoringClient();
 ...
 * 获取池风险评估
 */
router.get('/risk/:poolAddress', async (req, res, next) => {
    try {
 ...
 * 获取池健康状况
 */
router.get('/health/:poolAddress', async (req, res, next) => {
    try {
        const { poolAddress } = req.params;
        const scoringClient = service_manager_1.ServiceManager.getInstance().getScoringClient();
 ...
 * 批量获取池健康状况
 */
router.post('/health/bulk', async (req, res, next) => {
    try {
