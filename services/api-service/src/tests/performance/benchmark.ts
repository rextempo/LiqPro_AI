...
const TEST_CONFIG = {
  ...
  apiKey: process.env.API_KEY || config.apiKeys[0],
...
interface BenchmarkResult {
  ...
  totalMinResponseTime: number;
  totalMaxResponseTime: number;
  totalP95ResponseTime: number;
 ...
 * @returns 请求结果
 */
async function sendRequest(endpoint: typeof TEST_CONFIG.endpoints[0]): Promise<RequestResult> {
 ...
 * @returns 请求结果数组
 */
async function runIteration(isWarmup: boolean): Promise<RequestResult[]> {
 ...
 * @returns 基准测试结果
 */
function analyzeResults(allResults: RequestResult[]): BenchmarkResult {
  // 按端点分组
  ...
  for (const endpoint of TEST_CONFIG.endpoints) {
    ...
    const responseSizes = results
      ...
      .map(r => r.responseSize as number);
...
function saveResults(result: BenchmarkResult): void {
  const outputPath = path.resolve(process.cwd(), TEST_CONFIG.outputFile);
