...
// 当前活动的故障
let activeFault: string | undefined = undefined;
 ...
 * @returns 测试结果
 */
async function sendRequest(endpoint: string): Promise<TestResult> {
...
async function runRecoveryTest() {
  ...
  // 按故障类型分组统计
  const faultStats: Record<string, { requests: number, success: number, avgTime: number }> = {
