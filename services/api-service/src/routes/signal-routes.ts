...
import { SignalFilterOptions } from '../clients/signal-client';
...
router.get('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const signalClient = ServiceManager.getInstance().getSignalClient();
    
    // 从查询参数构建过滤选项
    const filterOptions: SignalFilterOptions = {};
    
    if (req.query.poolAddresses) {
      filterOptions.poolAddresses = (req.query.poolAddresses as string).split(',');
    }
    
    if (req.query.signalTypes) {
      filterOptions.signalTypes = (req.query.signalTypes as string).split(',') as any[];
    }
    
    if (req.query.minStrength) {
      filterOptions.minStrength = parseInt(req.query.minStrength as string, 10);
    }
    
    if (req.query.timeframes) {
      filterOptions.timeframes = (req.query.timeframes as string).split(',') as any[];
    }
    
    if (req.query.minReliability) {
      filterOptions.minReliability = parseInt(req.query.minReliability as string, 10);
    }
 ...
 *         description: Server error
 */
router.get('/pool/:address', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { address } = req.params;
    const signalClient = ServiceManager.getInstance().getSignalClient();
    
    // 从查询参数构建过滤选项
    const filterOptions: Omit<SignalFilterOptions, 'poolAddresses'> = {};
