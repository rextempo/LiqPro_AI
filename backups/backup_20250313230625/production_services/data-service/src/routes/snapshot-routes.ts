...
router.get('/:address/hourly', async (req, res) => {
  try {
    ...
    logger.error(`获取池 ${req.params.address} 的小时快照失败`, { error });
    res.status(500).json({
      status: 'error',
      message: '获取小时快照失败',
    });
...
router.get('/:address/daily', async (req, res) => {
  try {
    ...
    logger.error(`获取池 ${req.params.address} 的日快照失败`, { error });
    res.status(500).json({
      status: 'error',
      message: '获取日快照失败',
    });
...
router.get('/:address/weekly', async (req, res) => {
  try {
    ...
    logger.error(`获取池 ${req.params.address} 的周快照失败`, { error });
    res.status(500).json({
      status: 'error',
      message: '获取周快照失败',
    });
...
router.get('/:address/changes', async (req, res) => {
  try {
    ...
    logger.error(`获取池 ${req.params.address} 的变化率指标失败`, { error });
    res.status(500).json({
      status: 'error',
      message: '获取变化率指标失败',
    });
