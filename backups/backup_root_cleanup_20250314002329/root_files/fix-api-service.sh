...
# 创建server.js文件
cat > production/services/api-service/server.js << 'EOL'
...
# 创建.env文件
cat > production/services/api-service/.env << 'EOL'
PORT=3000
NODE_ENV=production
LOG_LEVEL=info
DATA_SERVICE_URL=http://data-service:3001
SIGNAL_SERVICE_URL=http://signal-service:3002
SCORING_SERVICE_URL=http://scoring-service:3003
AGENT_SERVICE_URL=http://agent-engine:3004
MONGODB_URI=mongodb://mongodb:27017/liqpro
EOL

# 创建启动脚本
cat > production/services/api-service/start.sh << 'EOL'
#!/bin/bash
