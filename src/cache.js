...
exports.prewarmCacheMiddleware = exports.clearCacheByTagsMiddleware = exports.clearCacheMiddleware = exports.cacheMiddleware = void 0;
const cache_service_1 = require("../services/cache-service");
const logger_1 = require("../utils/logger");
const config_1 = require("../config");
const logger = new logger_1.Logger('CacheMiddleware');
/**
 * 缓存中间件
 * 用于缓存API响应
 * @param options 缓存选项
...
const cacheMiddleware = (options = {}) => {
    ...
    const ttl = options.ttl || config_1.config.cache.defaultTtl;
 ...
 * @returns 中间件函数
 */
const clearCacheMiddleware = (keyPrefix, options = {}) => {
    const cacheService = cache_service_1.CacheService.getInstance();
    return async (req, res, next) => {
        try {
            // 在请求处理完成后清除缓存
 ...
 * @returns 中间件函数
 */
const clearCacheByTagsMiddleware = (tags, options = {}) => {
    const cacheService = cache_service_1.CacheService.getInstance();
    return async (req, res, next) => {
        try {
            // 在请求处理完成后清除缓存
...
exports.clearCacheByTagsMiddleware = clearCacheByTagsMiddleware;
/**
 * 缓存预热中间件
 ...
 * @returns 中间件函数
 */
const prewarmCacheMiddleware = (keyFetcher, options = {}) => {
    const cacheService = cache_service_1.CacheService.getInstance();
    return async (req, res, next) => {
        try {
            // 异步预热缓存，不阻塞请求
...
exports.prewarmCacheMiddleware = prewarmCacheMiddleware;
