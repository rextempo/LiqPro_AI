...
const { HourlySnapshot, DailySnapshot, WeeklySnapshot } = require('../models/pool-snapshots');
 ...
 */
async function generateHourlySnapshot(poolAddress, currentData) {
  try {
    const now = new Date();
    ...
    logger.error(`生成池 ${poolAddress} 的小时快照失败`, { error: error.message });
    throw error;
  }
}
 ...
 * @param {string} poolAddress 池地址
 */
async function generateDailySnapshot(poolAddress) {
  try {
    const now = new Date();
    const day = now.getUTCDate();
    ...
    logger.error(`生成池 ${poolAddress} 的日快照失败`, { error: error.message });
    throw error;
  }
}
 ...
 * @param {string} poolAddress 池地址
 */
async function generateWeeklySnapshot(poolAddress) {
  try {
    const now = new Date();
    // 获取当前是一年中的第几周
    ...
    logger.error(`生成池 ${poolAddress} 的周快照失败`, { error: error.message });
    throw error;
  }
}
 ...
 */
async function getHourlySnapshots(poolAddress, limit = 24, skip = 0) {
  try {
    ...
    logger.error(`获取池 ${poolAddress} 的小时快照失败`, { error: error.message });
    throw error;
  }
}
 ...
 * @param {number} skip 跳过数量
 */
async function getDailySnapshots(poolAddress, limit = 30, skip = 0) {
  try {
    ...
    logger.error(`获取池 ${poolAddress} 的日快照失败`, { error: error.message });
    throw error;
  }
}
 ...
 * @param {number} skip 跳过数量
 */
async function getWeeklySnapshots(poolAddress, limit = 12, skip = 0) {
  try {
    ...
    logger.error(`获取池 ${poolAddress} 的周快照失败`, { error: error.message });
    throw error;
  }
}
...
/**
 * 清理旧快照数据
 * @param {number} hoursToKeep 保留小时快照的小时数
 * @param {number} daysToKeep 保留日快照的天数
 * @param {number} weeksToKeep 保留周快照的周数
 */
async function cleanupOldSnapshots(hoursToKeep = 168, daysToKeep = 90, weeksToKeep = 52) {
  try {
    ...
    logger.error('清理旧快照数据失败', { error: error.message });
    throw error;
  }
}
