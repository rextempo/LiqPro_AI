...
const setupRabbitMQ = async () => {
  try {
    ...
    logger.info(`RabbitMQ 连接 URL: amqp://${RABBITMQ_USER}:***@${RABBITMQ_HOST}:${RABBITMQ_PORT}`);
...
const startServer = async () => {
  try {
    logger.info('信号服务启动中...');
    logger.info(`环境变量: PORT=${PORT}, RABBITMQ_HOST=${RABBITMQ_HOST}, RABBITMQ_USER=${RABBITMQ_USER}`);
